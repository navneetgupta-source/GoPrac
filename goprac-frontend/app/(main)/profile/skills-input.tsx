...
export function SkillsInput({ skillsList, onSkillsChange }: SkillsInputProps) {
  ...
  const fetchSuggestions = async (query: string) => {
    ...
    const filtered = (skillsList as any[]).filter(
      (skill) =>
        skill.name.toLowerCase().includes(query.toLowerCase()) &&
        !selectedSkills.some((selected) => selected.id === skill.id),
    )

      // console.log("Skill List:", filtered)
  ...
  const addCustomSkill = () => {
    ...
    const existingSuggestion = suggestions.find((s) => s.name.toLowerCase() === query.toLowerCase())

    if (existingSuggestion) {
      addSkill(existingSuggestion)
    } else {
      const customSkill = { id: negativeIdCounter--, name: query }
